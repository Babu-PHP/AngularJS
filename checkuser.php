...
$status = 'A';
 /* $sql_insert = $db->prepare("INSERT INTO users(name, emailid, username, password, status) VALUES (:name, :emailid, :username, :password, :status)");
        ...
        $sth = $db->prepare('SELECT name, colour, calories FROM users WHERE username = :username AND password = :colour');
$sth->bindParam(':username', $data->username, PDO::PARAM_STR, 12);
$sth->bindParam(':password', $data->password, PDO::PARAM_STR, 12);
//$sth->execute();
